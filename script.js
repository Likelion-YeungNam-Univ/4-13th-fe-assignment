...
const updateHistory = () => {
  ...
  historyList.forEach((history) => {
    ...
    deleteBtn.addEventListener("click", () => {
      deleteHistory(historyList.indexOf(history)); // .indexOf()는 배열에서 요소의 위치를 찾거나 존재 여부를 확인할 때 유용
    });
...
// forEach()를 사용해서 NodeList 각 버튼마다 처리하도록, nBtn은 현재 클릭된 num-btn 버튼
numBtn.forEach((nBtn) => {
  nBtn.addEventListener("click", () => {
    // 만약 누른 버튼이 C 라면
    // 누르면 기록까지 전부 다 없어져야 함 -> 모든 값 초기화 후 최신화
    if (nBtn.innerText == "C") {
      formula = "";
      resultNum = 0;
      historyList = [];
...
// forEach()를 사용해서 NodeList 각 버튼마다 처리하도록, cBtn은 현재 클릭된 calc-btn 버튼
